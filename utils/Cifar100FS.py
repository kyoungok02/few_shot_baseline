            ...
            for cls in selected_cls:
                ...
                for i, img in enumerate(selected_imgs_idx):
                    ...
                    if i % 2 == 1:
                        ...
                        all_image1 = os.path.join(image_path,image1)
                        all_image2 = os.path.join(image_path,image2)
                        label = 0.0
                    else :
                        other_cls = np.random.choice(self.classes)
                        while other_cls == cls:
                            other_cls = np.random.choice(self.classes)
                        other_path = os.path.join(self.image_dir,other_cls)
                        image1 = img
                        image2 = np.random.choice(os.listdir(other_path))
                        all_image1 = os.path.join(image_path,image1)
                        all_image2 = os.path.join(other_path,image2)
                        label = 1.0
                    induce_x1.append(all_image1)
                    induce_x2.append(all_image2)
